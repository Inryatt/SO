...
int main (int argc, char *argv[])
{
    ...
    vSet(0);      //zona crítica-está a manipular a variável value
    ...
    printf ("Final value = %d\n", vGet());    //zona crítica-está a manipular a variável value
...
/*
 * thread routine -- is this zona crítica?
 * 
 */
void *incrementer (void *arg)
{
    ...
    n = *((int *) arg);
    for (i = 0; i < n; i++) {
        vInc ();        //zona crítica-esá a manipular a variável value
    }
...
/* respostas ao guião 9
  
i) qual e a zona partilhada:
   R: é a int 'value'

    ii) 
    zonas critica sao zonas de codigo que manipulam a zona partilhada e 
    devem ser corridas em modo de exculsao multipla 
        -- qualquer coisa que manipule 'value'
*/