...
static int value = 0;         //zona crítica-está a manipular a variável value

/* set the variable value */
void vSet (int new_value)
{
    value = new_value;    //zona crítica-está a manipular a variável value
}

/* get the variable value */
int vGet (void)
{
    return value;     //zona crítica-está a manipular a variável value
}

/* increment the variable value */
void vInc (void)
{
    int val;      //zona crítica-está a manipular a variável value

    /* load locally the stored value */
    val = value;          //zona crítica-está a manipular a variável value

    /* generate time delay 1 */
    delay (BIG);          //zona crítica-está a manipular a variável value

    /* increment the local value and store it back */
    value = val + 1;          //zona crítica-está a manipular a variável value
